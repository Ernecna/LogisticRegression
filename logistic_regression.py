...
## AYKIRI DEĞERLERİ BELİRLEMEK VE DEĞİŞTİRMEK İÇİN KULLANACAĞIMIZ FONKSİYONLAR
...
# Veri setini ve hedef değişkeni tanımla
y = df["Outcome"]
X = df.drop(["Outcome"], axis=1)

# Eğitilmemiş bir LogisticRegression model nesnesi oluştur
log_model = LogisticRegression(max_iter=1000).fit(X,y)  # Bazı durumlarda, varsayılan iterasyon sayısı yetersiz kalabilir

# 10-kat çapraz doğrulama ile modeli değerlendir
cv_results = cross_validate(log_model,
                            X, y,
                            cv=10,  # 10 kat çapraz doğrulama
                            scoring=["accuracy", "precision", "recall", "f1", "roc_auc"],
                            return_train_score=False)

# Değerlendirme metriklerinin ortalamalarını yazdır
